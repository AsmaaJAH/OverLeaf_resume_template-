%-------------------------------------------------------------------------------
% SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Experience}


%-------------------------------------------------------------------------------
% CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------
  \cventry
    {SW Engineering Intern} % Job title
    {Inova LLC - Mobile and Web Development Corporation} % Organisation
    {Alexandria EG, UAE, KSA} % Location
    {July 2023 - Oct 2023} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Frontend using Flutter/Dart Android and IOS mobile apps, State management using 
        (provider \& Riverpod) packages, data-flow controllers.}
        \item {Backend using Firebase, Firestore, and Postman RESTful APIs.}
        \item {Integration teamwork brings together project managers, UX/UI designers, backend developers, and Flutter team through virtual and on-site meetings.}
        \item {\textbf{Technical Skills:} Meet tasks deadlines using Native features, Flutter packages e.g.:(http, image picker,uuid, dio, ..,etc)., and common platforms, like: Firebase, Bitbucket, Figma, Postman, Jira as an Agile project management tool, and Slack.Including CI/CD maintenance, localization, dark mode, app environments:( development, staging, production), and introducing flavors.}
        \item {\textbf{Soft Skills:} Teamwork, Time Management, Communication, Presentation skills.}
      \end{cvitems}
    }

\end{cventries}


%-------------------------------------------------------------------------------
\begin{cventries}

  %---------------------------------------------------------
    \cventry
      {SW Engineering Intern} % Job title
      {AIT Systems Corporation for web design, cloud hosting, and development services.} % Organisation
      {Alexandria EG} % Location
      {Aug 2022 - Sep 2022} % Date(s)
      {
        \begin{cvitems} % Description(s) of tasks/responsibilities
          \item {Web development, security, visual design, and mobile app in the SPS smart parking system project.}
          \item {Coordinating with AASTMT, ITIDA, and Alex Uni. using: (Flutter WebView, MySQL, PHP, Laravel, WordPress, CCtalk protocol, MQTT protocol, Node-RED) in the SW part and (AWS clouding servers, PV cells, Raspberry Pi, and coin and cash acceptors, additional larger display Screen) in the HW part.}
    \end{cvitems}
      }
  
  \end{cventries}
%-------------------------------------------------------------------------------
\begin{cventries}

  %---------------------------------------------------------
    \cventry
      {GRIP(Graduate Rotational Internship Program)} % Job title
      {The Sparks Foundation} % Organisation
      {Remotely, India and Singapore} % Location
      {Feb 2022 - April 2022} % Date(s)
      {
        \begin{cvitems} % Description(s) of tasks/responsibilities
          \item {Python codes to collect, analyze data, Build \& tune models.Colab ML Classification, and AI Search Algorithms. }
    \end{cvitems}
      }
  
  \end{cventries}

%-------------------------------------------------------------------------------
\begin{cventries}

  %---------------------------------------------------------
    \cventry
      {IT and Network intern } % Job title
      {AMOC "Alexandria Mineral Oils Corporate"} % Organisation
      {Egypt} % Location
      {Sep 2021} % Date(s)
      {
        \begin{cvitems} % Description(s) of tasks/responsibilities
          \item {Two Weeks intern at the SW and Network Department.}
    \end{cvitems}
      }
  
  \end{cventries}
%-------------------------------------------------------------------------------
\begin{cventries}

  %---------------------------------------------------------
    \cventry
      {Academic Visit} % Job title
      {The main Alexandrian Central of WE company.} % Organisation
      {Alexandria, Egypt} % Location
      {Jan 2020} % Date(s)
      {
        \begin{cvitems} % Description(s) of tasks/responsibilities
          \item {Monitored and Troubleshooted for Networks and Servers.}
    \end{cvitems}
      }
  
  \end{cventries}
  %-------------------------------------------------------------------------------
\begin{cventries}

  %---------------------------------------------------------
    \cventry
      {Repairing and Maintenance Trainee} % Job title
      {“TARGET” For Importing and Exporting Electronic Devices.} % Organisation
      {Egypt} % Location
      {2019} % Date(s)
      {
        \begin{cvitems} % Description(s) of tasks/responsibilities
          \item {Responsibility for the HW and SW Repairing and Maintenance of the imported electronics devices.}
    \end{cvitems}
      }
  \end{cventries}


%-------------------------------------------------------------------------------
\begin{cventries}

  %---------------------------------------------------------
    \cventry
      {Technical Trainee} % Job title
      {Etisalat office "the Arab Group".} % Organisation
      {Egypt} % Location
      {2018} % Date(s)
      {
        \begin{cvitems} % Description(s) of tasks/responsibilities
          \item {Responsibility for the office system administration.}
    \end{cvitems}
      }
  \end{cventries}


%-------------------------------------------------------------------------------
\begin{cventries}

  %---------------------------------------------------------
    \cventry
      {A trainee and main Researcher} % Job title
      {PHD-17 Team2, A Plus association.} % Organisation
      {FOE, Egypt} % Location
      {2017} % Date(s)
      {
        \begin{cvitems} % Description(s) of tasks/responsibilities
          \item {The “BCI and EMG using EEG technique” review paper. Our team won first place in this student activity.}
    \end{cvitems}
      }
  \end{cventries}


%-------------------------------------------------------------------------------
\begin{cventries}

  %---------------------------------------------------------
    \cventry
      {Full Scholarship To study STEM As Foundation Courses} % Job title
      {Zewail City for Science and Technology.} % Organisation
      {6th October, Cairo EG} % Location
      {2015 -  2016} % Date(s)
      {
        \begin{cvitems} % Description(s) of tasks/responsibilities
          \item {Won 4th place in ZCSF fair and conference,2015, in the category of College students’ scientific proposals, although I was still in secondary school.}
    \end{cvitems}
      }
  \end{cventries}


%-------------------------------------------------------------------------------
\begin{cventries}

  %---------------------------------------------------------
    \cventry
      {Electric Training For The Robotics Team } % Job title
      {YESBU/LYM In Bibliotheca Alexandrina} % Organisation
      {Egypt} % Location
      {2014 - 2015} % Date(s)
      {
        \begin{cvitems} % Description(s) of tasks/responsibilities
          \item {SW and HW preparation for the ROV submarine team.}
    \end{cvitems}
      }
  \end{cventries}


%-------------------------------------------------------------------------------
\begin{cventries}

  %---------------------------------------------------------
    \cventry
      {Six weeks Of C++ Language Training} % Job title
      {The Arab Academy for Science, Technology and Maritime Transport (AASTMT)} % Organisation
      {Egypt} % Location
      {2014} % Date(s)
      {
        \begin{cvitems} % Description(s) of tasks/responsibilities
          \item {Prepared for the ACM and IOI ( International Olympiad in Informatics for secondary school students).}
    \end{cvitems}
      }
  \end{cventries}
